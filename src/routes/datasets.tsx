      ...
      <div className="mx-auto max-w-[1500px] space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="rounded-2xl border border-border bg-card p-6 shadow-soft">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted-foreground">Repository Overview</div>
              <div className="mt-1 text-lg font-semibold tracking-tight">Enterprise Procurement Analytics — Engineering Showcase</div>
            </div>
            <a
              href="https://github.com/hrishik2907"
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-2 rounded-lg border border-border bg-background px-3 py-2 text-xs font-semibold transition hover:bg-surface"
            >
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-3">
            <div className="rounded-xl border border-border bg-surface p-4">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Languages</div>
              <div className="mt-3 flex flex-wrap gap-1.5">
                {["SQL", "PySpark", "Power BI", "React", "TypeScript", "DAX"].map((l) => (
                  <span key={l} className="rounded-full border border-border bg-background px-2.5 py-1 text-[11px] font-semibold text-muted-foreground">{l}</span>
                ))}
              </div>
            </div>
            <div className="rounded-xl border border-border bg-surface p-4">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Repository Structure</div>
              <pre className="mt-3 rounded-lg bg-background p-3 text-[11px] leading-relaxed text-foreground">{`/data
/sql
/pyspark
/powerbi
/react
/assets`}</pre>
            ...
            <div className="rounded-xl border border-border bg-surface p-4">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">Project Highlights</div>
              <ul className="mt-3 grid grid-cols-1 gap-1 text-xs">
                {["End-to-End ETL", "Procurement Dashboard", "Executive Reporting", "AI Procurement Analyst", "Supplier Risk Engine", "Power BI Service", "Responsive React Application", "Enterprise Dataset"].map((h) => (
                  <li key={h} className="flex items-center gap-1.5"><CheckCircle2 className="h-3 w-3 text-success" />{h}</li>
                ))}
              </ul>
            </div>
          </div>
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-8">
            {[
              ...
              ["5,248", "Rows"],
              ["300", "Suppliers"],
              ["1,209", "Purchase Orders"],
              ["₹170 Cr", "Contracts"],
              ["2", "Dashboards"],
              ["48", "DAX Measures"],
              ["14", "SQL / PySpark Scripts"],
            ].map(([k, v]) => (
              <div key={v} className="rounded-lg border border-border bg-surface p-3 text-center">
